...
# Convert to plotly
flood.freq1 <- plotly::ggplotly(flood.freq1, tooltip = "text")
...
#Convert to plotly
flood.freq2 <- plotly::ggplotly(flood.freq2, tooltip = "text")
...
# Convert to plotly
flood.dur1 <- plotly::ggplotly(flood.dur1, tooltip = "text")
...
#Convert to plotly
flood.dur2 <- plotly::ggplotly(flood.dur2, tooltip = "text")
...
# Convert to plotly
pond.freq1 <- plotly::ggplotly(pond.freq1, tooltip = "text")
...
#Convert to plotly
pond.freq2 <- plotly::ggplotly(pond.freq2, tooltip = "text")
...
# Convert to plotly
pond.dur1 <- plotly::ggplotly(pond.dur1, tooltip = "text")
...
#Convert to plotly
pond.dur2 <- plotly::ggplotly(pond.dur2, tooltip = "text")
...
el_h1 <- ggplot2::ggplot(elev.df |>  dplyr::filter(elev == "elev_h_ft"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2, color = "black", fill = "grey") + 
...
# Convert to plotly
comp1 <- plotly::ggplotly(comp1, tooltip = "text")
...
#Convert to plotly
comp2 <- plotly::ggplotly(comp2, tooltip = "text")
...
# Convert to plotly
drainage.class1 <- plotly::ggplotly(drainage.class1, tooltip = "text")
...
#Convert to plotly
drainage.class2 <- plotly::ggplotly(drainage.class2, tooltip = "text")
...
# Convert to plotly
perm.class1 <- plotly::ggplotly(perm.class1, tooltip = "text")
...
#Convert to plotly
perm.class2 <- plotly::ggplotly(perm.class2, tooltip = "text")
...
Depth to restriction prep code - this is the depth to the top of the horizon with a restriction. 
```{r, message=FALSE}
...
if (all(is.na(EDIT_updates_filtered$restrict_kind))) {print("No data populated in NASIS for this field.")
  ...
  EDIT_updates_filtered |>  dplyr::select(coiid, restrict_kind, comp_acres) |>  dplyr::group_by(restrict_kind) |>  
    dplyr::summarise(acres = sum(comp_acres),  '# components' = dplyr::n()) |>  dplyr::arrange(desc(acres)) |>  as.data.frame()
...
soil_depth1 <- plotly::ggplotly(soil_depth1, tooltip = "text")
...
soil_depth2 <- plotly::ggplotly(soil_depth2, tooltip = "text")
...
Organic matter depth is not a field in EDIT. Organic matter depth is provided for your own information, and will be referenced later association with subsurface fragments. 
...
o_horizon1 <- plotly::ggplotly(o_horizon1, tooltip = "text")
...
o_horizon2 <- plotly::ggplotly(o_horizon2, tooltip = "text")
...
The calculation of subsurface fragments is more difficult than other properties in this report for several reasons. Firstly, low and high values for different fragment sizes are independent in NASIS, but are not in-situ. For example, rock fragments over 3 inches (75 mm) include cobbles, stones, and boulders. Each fragment size has an independent low and high range. Cobbles could range from a low of 20% to a high of 50%, stones could range from 15-30%, and boulders could range from 20-40%. If you added the high ranges together it would equal 120%. This is of course impossible. In NASIS the ranges are independent of each other, but in-situ these values are correlated. It is unlikely that all fragment sizes will be at there maximum range at the same time, certainly not if it leads to a cumulative volume over 100%. Similarly, it is unlikely that all fragment sizes will be at their lowest range at the same time. 

When cobbles and stones are high, boulders are low - or any other combination that accounts for that fact that all fragment classes are unlikely to be high at the same time and will never combine to greater than 100%. 

The depth range for subsurface fragments needs to be user-defined. Subsurface frags are calculated from below the O horizon(s) to top of the first restriction, with one exception. In Histosols, subsurface frags are calculated from the top of the soil to the first restriction - O horizons are included in the calculation. Subsurface fragments are omitted from non-Histosol soils because rock fragments in those O horizons can be incompletely populated and inaccurate. 
...
Use as depth top - this value is the median of the acreage weighted component RV depth of O horizon(s). Why is the component RV used instead of the combined component high/low as is done in the majority of this script? Organic horizon data comes from the component horizon table, and high/low values from the component horizon table are incompletely populated. 
```{r}
...
```
Use as bottom depth - this value is the median of the acreage weighted combined component high/low depth to restriction: 
```{r}
if (all(is.na(EDIT_updates_filtered$restrict_depth_h_in)) &
    all(is.na(EDIT_updates_filtered$restrict_depth_l_in))) {print("No restriction depth populated in NASIS.")
...
subsurf_frag_h1 <- ggplot2::ggplot(subsurf_frags.df |>  dplyr::filter(frag == "subsurf_frags_hi_2mm.3in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  color = "black",
  fill = "grey") + 
  ggplot2::ggtitle("Subsurface frag high (2 mm. - 3 inches)") + 
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
...
subsurf_frags_l2 <- ggplot2::ggplot(subsurf_frags.df |>  dplyr::filter(frag == "subsurf_frags_lo_2mm.3in"), ggplot2::aes(x = value)) + 
  ...
  color = "black",
  fill = "grey") + 
  ggplot2::ggtitle("Subsurface frag low (2 mm. - 3 inches)") + 
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of acres") + ggplot2::xlab("Subsurface frag (%)") +
    ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  breaks = my_seq[seq(1, length(my_seq), by = 2)])
...
subsurf_frag_r1 <- ggplot2::ggplot(subsurf_frags.df |>  dplyr::filter(frag %in% c("subsurf_frags_hi_2mm.3in", "subsurf_frags_lo_2mm.3in")), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  color = "black",
  fill = "grey") + 
  ggplot2::ggtitle("Subsurface frag rep. (2 mm. - 3 inches)") + 
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("Subsurface frag (%)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  breaks = my_seq[seq(1, length(my_seq), by = 2)])
...
# my_vector <- c(1, 2, 5, 10, 20)
# 
# my_factor <- diff(range(c(EDIT_updates_filtered$sieve_subsurf_frags_hi_2mm.3in, 
#              EDIT_updates_filtered$sieve_subsurf_frags_lo_2mm.3in), na.rm = TRUE))/my_vector
# 
# reducer <- my_factor < 25
# 
# bin_width <- my_vector[reducer][which.min(abs(my_factor[reducer] - 15))]
...
# my_vector <- c(1, 2, 5, 10, 20)
# 
# my_factor <- diff(range(c(EDIT_updates_filtered$sieve_subsurf_frags_hi_gt_3in, 
#              EDIT_updates_filtered$sieve_subsurf_frags_lo_gt_3in), na.rm = TRUE))/my_vector
# 
# reducer <- my_factor < 25
# 
# bin_width <- my_vector[reducer][which.min(abs(my_factor[reducer] - 15))]
