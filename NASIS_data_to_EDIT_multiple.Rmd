...
This report summarizes data from components correlated to an ecosite. It could be used to populate EDIT, to QC component-ecosite correlations, or as documentation of ecosite properties. Of particular importance to QC, the user can hover their cursor over figures to see what component ids (coiids) are being represented. This allows you to hover over values that look like outliers and identify which coiid(s) might be improperly correlated.

**# of components vs. component acreage:**

The following analysis provides two ways of looking at data - (1) by component and (2) by component acreage. Looking at components (1) means that we treat all components equally, regardless of their associated acreage. Looking at data by component acreage (2) acknowledges that components may differ in their importance based on their acreage. Therefore, you will see figures below that summarize data both by component (1) and by component acreage (2). 

I would suggest that looking at data by components (1) is the best way to QC your data. The correlation of components to ecosites should be assessed based on the *similarity of properties* between the ecosite and component. A bad component-ecosite correlation can just as easily happen for a component with low acreage as it can for a component with high acreage. For the QC process, you should think of component acreage as a metric of how important it is to have a good component-ecosite correlation. A bad component-ecosite correlation for a high acreage component will be more disruptive to the ecosite concept than a bad component-ecosite correlation for a low acreage component because component acreage is used to weight standard calculations. 

**Standardized calculations:**

This report provides a standardized way of populating the range of properties for an ecosite. 


This is accomplished by weighting components by their acreage, hence why good correlations for high acreage components are important. Component properties are replicated by their associated acres. After replication, the following calculations are used:
...
Notice that RV values are not used in any of these calculations. This is intentional - RV values may not be truly representative for all properties of a component. This is due to the fact that component RV values are often times populated from a representative component pedon, and no pedon perfectly exemplifies the overall concept. In theory, the best available pedon is used, but it may be abnormal in some of its properties. 
...
NA_acreage <- ifelse(length(NA_acreage) == 0, "No components missing acreage.", NA_acreage)


acreage_zero <- paste(EDIT_updates_filtered |> dplyr::filter(comp_acres == 0) |> dplyr::pull(coiid))
acreage_zero <- ifelse(length(acreage_zero) == 0, "No components with zero acreage.", acreage_zero)
...
Table of correlated components and associated acreage
```{r, message=FALSE, warning=FALSE}
if (all(is.na(EDIT_updates_filtered$compname))) {print("No data populated in NASIS for this field.")
...
if (all(is.na(EDIT_updates_filtered$landform)) | 
    ...
    landform.df <- EDIT_updates_filtered |> dplyr::select(landform, comp_acres, coiid, compname) |> dplyr::group_by(landform) |> 
    dplyr::summarise(acres = sum(comp_acres), `# components` = dplyr::n(), coiids = toString(coiid),
...
if (all(is.na(EDIT_updates_filtered$flood_freq))) {print("No data populated in NASIS for this field.")
  ...
  flood.freq.df <- EDIT_updates_filtered |>  dplyr::select(coiid, flood_freq, comp_acres, compname)  
...
if (all(is.na(EDIT_updates_filtered$flood_dur))) {print("No data populated in NASIS for this field.")
  ...
  flood.dur.df <- EDIT_updates_filtered |>  dplyr::select(coiid, flood_dur, comp_acres, compname)  
...
if (all(is.na(EDIT_updates_filtered$pond_freq))) {print("No data populated in NASIS for this field.")
  ...
  pond.freq.df <- EDIT_updates_filtered |>  dplyr::select(coiid, pond_freq, comp_acres, compname)  
...
if (all(is.na(EDIT_updates_filtered$pond_dur))) {print("No data populated in NASIS for this field.")
  ...
  pond.dur.df <- EDIT_updates_filtered |>  dplyr::select(coiid, pond_dur, comp_acres, compname)  
...
elev.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, elev_h_ft, elev_l_ft, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "elev",
    values_to = "value",
...
el_h2 <- ggplot2::ggplot(elev.df |>  dplyr::filter(elev == "elev_h_ft"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
      "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
...
el_l2 <- ggplot2::ggplot(elev.df |>  dplyr::filter(elev == "elev_l_ft"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
      "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
slope.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, slope_h, slope_l, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "slope",
    values_to = "value",
...
slope_h2 <- ggplot2::ggplot(slope.df |>  dplyr::filter(slope == "slope_h"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
      "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
slope_l2 <- ggplot2::ggplot(slope.df |>  dplyr::filter(slope == "slope_l"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
      "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
if (all(is.na(EDIT_updates_filtered$slope_shp_across_down))) {print("No data populated in NASIS for this field.")
    ...
    EDIT_updates_filtered |> dplyr::select(slope_shp_across_down, comp_acres, coiid, compname) |> dplyr::group_by(slope_shp_across_down) |> 
    dplyr::summarise(acres = sum(comp_acres), `# components` = dplyr::n(), coiids = toString(coiid),
                     compnames = toString(compname)) |> 
    dplyr::arrange(dplyr::desc(acres)) |> knitr::kable()
...
hillslope <- EDIT_updates_filtered |> dplyr::select(hillslope_profile, coiid, comp_acres, compname)
...
if (all(is.na(EDIT_updates_filtered$hillslope_profile))) {print("No data populated in NASIS for this field.")
    ...
    EDIT_updates_filtered |> dplyr::select(hillslope_profile, comp_acres, coiid, compname) |> dplyr::group_by(hillslope_profile) |> 
    dplyr::summarise(acres = sum(comp_acres), `# components` = dplyr::n(), coiids = toString(coiid),
                     compnames = toString(compname)) |> 
    dplyr::arrange(dplyr::desc(acres)) |> knitr::kable()
...
pond_depth.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, pond_depth_l, pond_depth_h, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "pond_depth",
    values_to = "value",
...
pond_depth_h2 <- ggplot2::ggplot(pond_depth.df |>  dplyr::filter(pond_depth == "pond_depth_h"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
pond_depth_l2 <- ggplot2::ggplot(pond_depth.df |>  dplyr::filter(pond_depth == "pond_depth_l"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
water_table.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, water_table_l_in, water_table_h_in, comp_acres, compname) |> 
  tidyr::pivot_longer(
...
water_table_h2 <- ggplot2::ggplot(water_table.df |>  dplyr::filter(water_table == "water_table_h_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
water_table_l2 <- ggplot2::ggplot(water_table.df |>  dplyr::filter(water_table == "water_table_l_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
aspect.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, aspect_rep, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "aspect",
    values_to = "value",
    cols = c("aspect_rep")) |> 
...
aspect_r2 <- ggplot2::ggplot(aspect.df |>  dplyr::filter(aspect == "aspect_rep"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
map.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, map_h_in, map_l_in, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "map",
    values_to = "value",
...
map_h2 <- ggplot2::ggplot(map.df |>  dplyr::filter(map == "map_h_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
map_l2 <- ggplot2::ggplot(map.df |>  dplyr::filter(map == "map_l_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
ffd.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, ffd_h, ffd_l, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "ffd",
    values_to = "value",
    cols = c("ffd_h", "ffd_l")) |> 
...
ffd_h2 <- ggplot2::ggplot(ffd.df |>  dplyr::filter(ffd == "ffd_h"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
ffd_l2 <- ggplot2::ggplot(ffd.df |>  dplyr::filter(ffd == "ffd_l"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
if (all(is.na(EDIT_updates_filtered$parent_material))) {print("No data populated in NASIS for this field.")
    ...
    EDIT_updates_filtered |> dplyr::select(parent_material, comp_acres, coiid, compname) |> dplyr::group_by(parent_material) |> 
    dplyr::summarise(acres = sum(comp_acres), `# components` = dplyr::n(), coiids = toString(coiid),
                     compnames = toString(compname)) |> 
...
if (all(is.na(EDIT_updates_filtered$surf_texture))) {print("No data populated in NASIS for this field.")
  ...
  if (length(unique(EDIT_updates_filtered$surf_texture <= 15))) {
    ...
    comp.df <- EDIT_updates_filtered |>  dplyr::select(coiid, surf_texture, comp_acres, compname)  
...
my_levels <-  c(
  ...
  drainage.class.df <- EDIT_updates_filtered |>  dplyr::select(coiid, drainage_class, comp_acres, compname)  
...
if (all(is.na(EDIT_updates_filtered$ksat_l_0_40_in))) {print("No data populated in NASIS for this field.")
  ...
  perm.class.df <- EDIT_updates_filtered |>  dplyr::select(coiid, ksat_l_0_40_in, comp_acres, compname)  
...
perm.class1 <- ggplot2::ggplot(perm.class.df, ggplot2::aes(ksat_l_0_40_in)) + 
  ggplot2::geom_bar(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname)),
                    color = "black", fill = "grey", closed = "left") + 
  ggplot2::ggtitle("Ksat class") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("Ksat class")
...
perm.class2 <- ggplot2::ggplot(perm.class.df, ggplot2::aes(ksat_l_0_40_in, comp_acres)) +
  ggplot2::geom_col(ggplot2::aes(text = paste("coiid(s):", coiid,
                    ...
                    color = "black", fill = "grey", closed = "left") + 
  ggplot2::ggtitle("Ksat class") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of acres") + ggplot2::xlab("Ksat class")
...
if (all(is.na(EDIT_updates_filtered$ksat_l_0_40_in))) {print("No data populated in NASIS for this field.")
  ...
  EDIT_updates_filtered |>  dplyr::select(coiid, ksat_l_0_40_in, comp_acres) |>  dplyr::group_by(ksat_l_0_40_in) |>  
    dplyr::summarise(acres = sum(comp_acres),  '# components' = dplyr::n()) |>  dplyr::arrange(desc(acres)) |>  as.data.frame()

}
```

**Ksat acreage weighted percentiles**
...
if (all(is.na(EDIT_updates_filtered$ksat_l_0_40_in))) {print("No data populated in NASIS for this field.")
    ...
    expanded_perm_class <- rep(EDIT_updates_filtered$ksat_l_0_40_in, EDIT_updates_filtered$comp_acres)
...
if (all(is.na(EDIT_updates_filtered$perm_1_0_40_in))) {print("No data populated in NASIS for this field.")
  ...
  perm.class.df <- EDIT_updates_filtered |>  dplyr::select(coiid, perm_1_0_40_in, comp_acres, compname)  
  
# Component histogram 
perm.class1 <- ggplot2::ggplot(perm.class.df, ggplot2::aes(perm_1_0_40_in)) + 
  ggplot2::geom_bar(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname)),
                    color = "black", fill = "grey", closed = "left") + 
...
# Acreage histogram
perm.class2 <- ggplot2::ggplot(perm.class.df, ggplot2::aes(perm_1_0_40_in, comp_acres)) +
  ggplot2::geom_col(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname)),
                    color = "black", fill = "grey", closed = "left") + 
  ggplot2::ggtitle("Permeability class") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
...
if (all(is.na(EDIT_updates_filtered$perm_1_0_40_in))) {print("No data populated in NASIS for this field.")
  ...
  EDIT_updates_filtered |>  dplyr::select(coiid, perm_1_0_40_in, comp_acres) |>  dplyr::group_by(perm_1_0_40_in) |>  
    dplyr::summarise(acres = sum(comp_acres),  '# components' = dplyr::n()) |>  dplyr::arrange(desc(acres)) |>  as.data.frame()
...
if (all(is.na(EDIT_updates_filtered$perm_1_0_40_in))) {print("No data populated in NASIS for this field.")
    ...
    expanded_perm_class <- rep(EDIT_updates_filtered$perm_1_0_40_in, EDIT_updates_filtered$comp_acres)
    expanded_perm_class <- expanded_perm_class[order(match(expanded_perm_class, my_levels))]
...
depth_classes <- c(0, 25, 50, 100, 150)

EDIT_updates_filtered$restrict_depth_h_in <- EDIT_updates_filtered$restrict_depth_h_in * 2.54
EDIT_updates_filtered$restrict_depth_l_in <- EDIT_updates_filtered$restrict_depth_l_in * 2.54
EDIT_updates_filtered$restrict_depth_r_in <- EDIT_updates_filtered$restrict_depth_r_in * 2.54
...
restriction.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, restrict_depth_h_in, restrict_depth_l_in, comp_acres, compname) |> 
  tidyr::pivot_longer(
...
restriction_h1 <- ggplot2::ggplot(restriction.df |>  dplyr::filter(restriction == "restrict_depth_h_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
  ...
  ggplot2::ylab("# of components") + ggplot2::xlab("Restriction depth (cm)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
...
restriction_h2 <- ggplot2::ggplot(restriction.df |>  dplyr::filter(restriction == "restrict_depth_h_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ggplot2::ggtitle("Restriction depth high") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of acres") + ggplot2::xlab("Restriction depth (cm)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
                              breaks = my_seq[seq(1, length(my_seq), by = 2)]) + ggplot2::geom_vline(xintercept = depth_classes, colour="red")
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_h_in))) {print("No data populated in NASIS for this field.")
    ...
    data.frame(meaurement = c("cm", "inches"), "95%" = 
             c(restriction_na_h_rem_expanded[ceiling(0.95 * length(restriction_na_h_rem_expanded))],
             restriction_na_h_rem_expanded[ceiling(0.95 * length(restriction_na_h_rem_expanded))] / 2.54), 
             check.names = FALSE)
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_h_in))) {print("No data populated in NASIS for this field. For restriction, this likely means no restriction was observed.")
  ...
  rest_h_cm <- summary(restriction_na_h_rem_expanded)
  rest_h_in <- summary(restriction_na_h_rem_expanded / 2.54)

  data.frame(Measurement = c("cm", "in"),
             Min = c(rest_h_cm[[1]],
                     rest_h_in[[1]]),
             "1st Qu." = c(rest_h_cm[[2]],
                     rest_h_in[[2]]),
             "Median" = c(rest_h_cm[[3]],
                     rest_h_in[[3]]),
             "Mean" = c(rest_h_cm[[4]],
                     rest_h_in[[4]]),
             "3rd Qu." = c(rest_h_cm[[5]],
                     rest_h_in[[5]]),
             "Max" = c(rest_h_cm[[6]],
                     rest_h_in[[6]]),
             check.names = FALSE
...
restriction_l2 <- ggplot2::ggplot(restriction.df |>  dplyr::filter(restriction == "restrict_depth_l_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_l_in))) {print("No data populated in NASIS for this field.")
    ...
    data.frame(meaurement = c("cm", "inches"), "5%" = 
                 c(restriction_na_l_rem_expanded[ceiling(0.05 * length(restriction_na_l_rem_expanded))],
                  restriction_na_l_rem_expanded[ceiling(0.05 * length(restriction_na_l_rem_expanded))] / 2.54), 
             check.names = FALSE)
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_l_in))) {print("No data populated in NASIS for this field. For restriction, this likely means no restriction was observed.")
  ...
  rest_l_cm <- summary(restriction_na_l_rem_expanded)
  rest_l_in <- summary(restriction_na_l_rem_expanded / 2.54)

  data.frame(Measurement = c("cm", "in"),
             Min = c(rest_l_cm[[1]],
                     rest_l_in[[1]]),
             "1st Qu." = c(rest_l_cm[[2]],
                     rest_l_in[[2]]),
             "Median" = c(rest_l_cm[[3]],
                     rest_l_in[[3]]),
             "Mean" = c(rest_l_cm[[4]],
                     rest_l_in[[4]]),
             "3rd Qu." = c(rest_l_cm[[5]],
                     rest_l_in[[5]]),
             "Max" = c(rest_l_cm[[6]],
                     rest_l_in[[6]]),
             check.names = FALSE
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_h_in)) &
  ...
  data.frame(meaurement = c("cm", "inches"), "20%" = 
               c(restriction_r_sum_stats[ceiling(0.2 * length(restriction_r_sum_stats))],
                 restriction_r_sum_stats[ceiling(0.2 * length(restriction_r_sum_stats))] / 2.54),
                "80%" = c(restriction_r_sum_stats[ceiling(0.8 * length(restriction_r_sum_stats))],
                          restriction_r_sum_stats[ceiling(0.8 * length(restriction_r_sum_stats))] / 2.54),
             check.names = FALSE)
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_h_in)) &
  ...
  rest_r_cm <- summary(restriction_r_sum_stats)
  rest_r_in <- summary(restriction_r_sum_stats / 2.54)

  data.frame(Measurement = c("cm", "in"),
             Min = c(rest_r_cm[[1]],
                     rest_r_in[[1]]),
             "1st Qu." = c(rest_r_cm[[2]],
                     rest_r_in[[2]]),
             "Median" = c(rest_r_cm[[3]],
                     rest_r_in[[3]]),
             "Mean" = c(rest_r_cm[[4]],
                     rest_r_in[[4]]),
             "3rd Qu." = c(rest_r_cm[[5]],
                     rest_r_in[[5]]),
             "Max" = c(rest_r_cm[[6]],
                     rest_r_in[[6]]),
             check.names = FALSE
...
if (all(is.na(EDIT_updates_filtered$restrict_kind))) {print("No data populated in NASIS for this field.")
  ...
  restrict.kind.df <- EDIT_updates_filtered |>  dplyr::select(coiid, restrict_kind, comp_acres, compname)  
  
  EDIT_updates_filtered |> dplyr::select(restrict_kind, comp_acres, coiid, compname) |> dplyr::group_by(restrict_kind) |> 
    dplyr::summarise(acres = sum(comp_acres), `# components` = dplyr::n(), coiids = toString(coiid),
                     compnames = toString(compname)) |> 
    dplyr::arrange(dplyr::desc(acres)) |> knitr::kable()
...
```{r, message=FALSE}
EDIT_updates_filtered$soil_depth_h_in <- EDIT_updates_filtered$soil_depth_h_in * 2.54
EDIT_updates_filtered$soil_depth_l_in <- EDIT_updates_filtered$soil_depth_l_in * 2.54
EDIT_updates_filtered$soil_depth_r_in <- EDIT_updates_filtered$soil_depth_r_in * 2.54
...
soil.depth.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, soil_depth_r_in, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "soil.depth",
    values_to = "value",
...
soil_depth1 <- ggplot2::ggplot(soil.depth.df |>  dplyr::filter(soil.depth ==  "soil_depth_h_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
  ...
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("Soil depth (cm)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  breaks = my_seq[seq(1, length(my_seq), by = 2)]) + ggplot2::geom_vline(xintercept = depth_classes, colour="red")
...
soil_depth2 <- ggplot2::ggplot(soil.depth.df |>  dplyr::filter(soil.depth ==  "soil_depth_h_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  ...
  ggplot2::ggtitle("Soil depth high") + 
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of acres") + ggplot2::xlab("Soil depth (cm)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  breaks = my_seq[seq(1, length(my_seq), by = 2)]) + ggplot2::geom_vline(xintercept = depth_classes, colour="red")
...
if (all(is.na(EDIT_updates_filtered$soil_depth_h_in))) {print("No data populated in NASIS for this field.")
  ...
  depth_h_cm <- summary(soil_depth_stats)
  depth_h_in <- summary(soil_depth_stats/2.54)

  data.frame(Measurement = c("cm", "in"),
             Min = c(depth_h_cm[[1]],
                     depth_h_in[[1]]),
             "1st Qu." = c(depth_h_cm[[2]],
                     depth_h_in[[2]]),
             "Median" = c(depth_h_cm[[3]],
                     depth_h_in[[3]]),
             "Mean" = c(depth_h_cm[[4]],
                     depth_h_in[[4]]),
             "3rd Qu." = c(depth_h_cm[[5]],
                     depth_h_in[[5]]),
             "Max" = c(depth_h_cm[[6]],
                     depth_h_in[[6]]),
             check.names = FALSE
...
soil_depth2 <- ggplot2::ggplot(soil.depth.df |>  dplyr::filter(soil.depth ==  "soil_depth_l_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
if (all(is.na(EDIT_updates_filtered$soil_depth_l_in))) {print("No data populated in NASIS for this field.")
  ...
  depth_l_cm <- summary(soil_depth_stats)
  depth_l_in <- summary(soil_depth_stats/2.54)

  data.frame(Measurement = c("cm", "in"),
             Min = c(depth_l_cm[[1]],
                     depth_l_in[[1]]),
             "1st Qu." = c(depth_l_cm[[2]],
                     depth_l_in[[2]]),
             "Median" = c(depth_l_cm[[3]],
                     depth_l_in[[3]]),
             "Mean" = c(depth_l_cm[[4]],
                     depth_l_in[[4]]),
             "3rd Qu." = c(depth_l_cm[[5]],
                     depth_l_in[[5]]),
             "Max" = c(depth_l_cm[[6]],
                     depth_l_in[[6]]),
             check.names = FALSE
...
soil_depth2 <- ggplot2::ggplot(soil.depth.df |>  dplyr::filter(soil.depth ==  "soil_depth_r_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
if (all(is.na(EDIT_updates_filtered$soil_depth_r_in))) {print("No data populated in NASIS for this field.")
  ...
  depth_r_cm <- summary(soil_depth_stats)
  depth_r_in <- summary(soil_depth_stats/2.54)

  data.frame(Measurement = c("cm", "in"),
             Min = c(depth_r_cm[[1]],
                     depth_r_in[[1]]),
             "1st Qu." = c(depth_r_cm[[2]],
                     depth_r_in[[2]]),
             "Median" = c(depth_r_cm[[3]],
                     depth_r_in[[3]]),
             "Mean" = c(depth_r_cm[[4]],
                     depth_r_in[[4]]),
             "3rd Qu." = c(depth_r_cm[[5]],
                     depth_r_in[[5]]),
             "Max" = c(depth_r_cm[[6]],
                     depth_r_in[[6]]),
             check.names = FALSE
...
surf_frag.df <-
  ...
  dplyr::select(coiid, frags_hi_2mm_3in_on_surf, frags_lo_2mm_3in_on_surf, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "surf_frag",
    values_to = "value",
...
surf_frag2 <- ggplot2::ggplot(surf_frag.df |>  dplyr::filter(surf_frag == "frags_hi_2mm_3in_on_surf"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
surf_frag2 <- ggplot2::ggplot(surf_frag.df |>  dplyr::filter(surf_frag == "frags_lo_2mm_3in_on_surf"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
surf_frag2 <- ggplot2::ggplot(surf_frag.df |>  dplyr::filter(surf_frag == "frags_hi_gt_3in_on_surf"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
surf_frag2 <- ggplot2::ggplot(surf_frag.df |>  dplyr::filter(surf_frag == "frags_lo_gt_3in_on_surf"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
awc.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, awc_l_0_40_in, awc_h_0_40_in, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "awc",
    values_to = "value",
...
awc_h2 <- ggplot2::ggplot(awc.df |>  dplyr::filter(awc == "awc_h_0_40_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weighted = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey", closed = "left") + 
...
awc_l2 <- ggplot2::ggplot(awc.df |>  dplyr::filter(awc == "awc_l_0_40_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid,
                                                  "<br>compname:", compname), weight = acres), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
...
ph.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, pH_h_0_10in, pH_l_0_10in, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "ph",
    values_to = "value",
...
ph.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, pH_h_10_40in, pH_l_10_40in, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "ph",
    values_to = "value",
...
subsurf_frags.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, 
                                   subsurf_frags_hi_2mm.3in, subsurf_frags_lo_2mm.3in, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "frag",
    values_to = "value",
...
subsurf_frags.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, subsurf_frags_lo_gt_3in,
                                   subsurf_frags_hi_gt_3in, comp_acres, compname) |> 
  tidyr::pivot_longer(
    names_to = "frag",
    values_to = "value",
