...
# divide the acres evenly between the two slope shapes. 
slope_shape$comp_acres2 <- ifelse(stringr::str_detect(slope_shape$slope_shp_across_down, ",") == TRUE,
                                  slope_shape$comp_acres/2, slope_shape$comp_acres)


# now we split the multiple slope shapes into singles, thereby making more rows
slope_shape2 <- slope_shape |> dplyr::filter(stringr::str_detect(slope_shape$slope_shp_across_down, ",") == TRUE)

slope_shape2$slope_shp_across_down <- slope_shape2$slope_shp_across_down |> 
  stringr::str_extract("(?<=,).+")
slope_shape2$slope_shp_across_down <- gsub(" ", "", slope_shape2$slope_shp_across_down)

slope_shape$slope_shp_across_down <- ifelse(stringr::str_detect(slope_shape$slope_shp_across_down, ",") == TRUE,
                                            slope_shape$slope_shp_across_down |> 
                                              stringr::str_extract(".+(?=,)"),
                                            slope_shape$slope_shp_across_down)

# now combine them and summarize
slope_shape_combined <- rbind(slope_shape, slope_shape2) |> 
...
# plot
ggplot(slope_shape_combined_all |> dplyr::select(-combined), aes(x = across, y = down)) +
  ggplot2::geom_raster(aes(fill = acres)) + 
  ggplot2::scale_fill_gradient(low = "grey90", high = "red") +
  labs(x = "across", y = "down", title = "Slope shape")
