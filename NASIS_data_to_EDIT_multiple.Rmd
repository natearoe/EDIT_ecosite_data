...
EDIT_updates <- read.csv(paste(pardir, "/ecosite_report.csv", sep = ""))
```
...
```{r  class.source = "fold-show", echo=FALSE}
EDIT_updates_filtered <- EDIT_updates |>  dplyr::filter(ecosite_id == "R022AX000CA")
...
# Data manipulation into sequence ranges
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- elev.df |>  dplyr::filter(elev %in% c("elev_h_ft", "elev_l_ft")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(elev) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

elev_acres <- do.call(rbind, my_list_summarised)

# Acreage unit

acreage_unit <- ifelse(min(elev_acres$acres[elev_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(elev_acres$acres[elev_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(elev_acres$acres[elev_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(elev_acres$acres[elev_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))

names(acreage_unit) <- ifelse(acreage_unit == 1, "number", ifelse(acreage_unit == 10, "tens", 
                        ifelse(acreage_unit == 100, "hundreds", 
                               ifelse(acreage_unit == 1000, "thousands", "tens of thousands"))))
...
elev_h_acres_prepped <- data.frame(elev = my_seq, row.names = NULL) |>  dplyr::rename(range = elev) |>                           dplyr::left_join(elev_acres |> dplyr::filter(elev == "elev_h_ft"))
...
expand_elev_h <- elev_h_acres_prepped[rep(1:nrow(elev_h_acres_prepped), times = plyr::round_any(elev_h_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "elev", "coiids"), drop = FALSE]

# Create acreage histogram

el_h2 <- ggplot2::ggplot(expand_elev_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Elevation high") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Elevation (ft.)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
elev_l_acres_prepped <- data.frame(elev = my_seq, row.names = NULL) |>  dplyr::rename(range = elev) |>                           dplyr::left_join(elev_acres |> dplyr::filter(elev == "elev_l_ft"))
...
expand_elev_l <- elev_l_acres_prepped[rep(1:nrow(elev_l_acres_prepped), times = plyr::round_any(elev_l_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "elev", "coiids"), drop = FALSE]

# Create acreage histogram

el_l2 <- ggplot2::ggplot(expand_elev_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
elev_r_acres_prepped <- data.frame(elev = my_seq, row.names = NULL) |>  dplyr::rename(range = elev) |>                           dplyr::left_join(elev_acres |> dplyr::filter(elev %in% c("elev_h_ft", "elev_l_ft")))
...
expand_elev_r <- elev_r_acres_prepped[rep(1:nrow(elev_r_acres_prepped), times = plyr::round_any(elev_r_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "elev", "coiids"), drop = FALSE]

# Create acreage histogram

el_r2 <- ggplot2::ggplot(expand_elev_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- slope.df |>  dplyr::filter(slope %in% c("slope_h", "slope_l")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(slope) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

slope_acres <- do.call(rbind, my_list_summarised) 

# Acreage unit
  
acreage_unit <- ifelse(min(slope_acres$acres[slope_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(slope_acres$acres[slope_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(slope_acres$acres[slope_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(slope_acres$acres[slope_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
slope_h_acres_prepped <- data.frame(slope = my_seq, row.names = NULL) |>  dplyr::rename(range = slope) |> 
                        dplyr::left_join(slope_acres |> dplyr::filter(slope == "slope_h"))
...
expand_slope_h <- slope_h_acres_prepped[rep(1:nrow(slope_h_acres_prepped), times = plyr::round_any(slope_h_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "slope", "coiids"), drop = FALSE]

# Create acreage histogram

slope_h2 <- ggplot2::ggplot(expand_slope_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Slope high") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Slope (%)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
slope_l_acres_prepped <- data.frame(slope = my_seq, row.names = NULL) |>  dplyr::rename(range = slope) |> 
                        dplyr::left_join(slope_acres |> dplyr::filter(slope == "slope_l"))
...
expand_slope_l <- slope_l_acres_prepped[rep(1:nrow(slope_l_acres_prepped), times = plyr::round_any(slope_l_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "slope", "coiids"), drop = FALSE]

# Create acreage histogram

slope_l2 <- ggplot2::ggplot(expand_slope_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
slope_r_acres_prepped <- data.frame(slope = my_seq, row.names = NULL) |>  dplyr::rename(range = slope) |> 
                        dplyr::left_join(slope_acres |> dplyr::filter(slope %in% c("slope_l", "slope_h")))
...
expand_slope_r <- slope_r_acres_prepped[rep(1:nrow(slope_r_acres_prepped), times = plyr::round_any(slope_r_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "slope", "coiids"), drop = FALSE]

# Create acreage histogram

slope_r2 <- ggplot2::ggplot(expand_slope_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
for(i in names(my_seq)){
  ...
  my_list[[i]] <- pond_depth.df |>  dplyr::filter(pond_depth %in% c("pond_depth_h", "pond_depth_l")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(pond_depth) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})


pond_depth_acres <- do.call(rbind, my_list_summarised) |> 
  dplyr::mutate(numb_coiids = sapply(coiids, FUN = function(x){
    x |> stringr::str_split(",") |> unlist() |> length()
  }))

pond_depth_acres_na <- pond_depth.df |> cbind(range = NA) |>  dplyr::filter(is.na(value)) |> dplyr::group_by(pond_depth, range) |>  
                                dplyr::summarise(acres = sum(acres), coiids = dplyr::n()) 

# Acreage unit

acreage_unit <- ifelse(min(pond_depth_acres$acres[pond_depth_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(pond_depth_acres$acres[pond_depth_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(pond_depth_acres$acres[pond_depth_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(pond_depth_acres$acres[pond_depth_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
pond_depth_h_acres_prepped <- data.frame(pond_depth = my_seq, row.names = NULL) |>  dplyr::rename(range = pond_depth) |> 
                        dplyr::left_join(pond_depth_acres |> dplyr::filter(pond_depth == "pond_depth_h"))

pond_depth_na_h <- rbind(pond_depth_h_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), pond_depth_acres_na |> dplyr::filter(pond_depth == "pond_depth_h"))
...
expand_pond_depth_h <- pond_depth_h_acres_prepped[rep(1:nrow(pond_depth_h_acres_prepped), times = plyr::round_any(pond_depth_h_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "pond_depth", "coiids"), drop = FALSE]

# Create acreage histogram

pond_depth_h2 <- ggplot2::ggplot(expand_pond_depth_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Ponding depth high") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Ponding depth (inches)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
if (all(is.na(EDIT_updates_filtered$pond_depth_h))) {print("No data populated in NASIS for this field.")
  ...
  pond_depth_sum_df <- pond_depth_na_h |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(pond_depth), pond_depth)
 
  pond_depth_sum_df |> dplyr::select(full_range, everything(), -range) 
...
pond_depth_l_acres_prepped <- data.frame(pond_depth = my_seq, row.names = NULL) |>  dplyr::rename(range = pond_depth) |> 
                        dplyr::left_join(pond_depth_acres |> dplyr::filter(pond_depth == "pond_depth_l"))

pond_depth_na_l <- rbind(pond_depth_l_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), pond_depth_acres_na |> dplyr::filter(pond_depth == "pond_depth_l"))
...
expand_pond_depth <- pond_depth_l_acres_prepped[rep(1:nrow(pond_depth_l_acres_prepped), times = plyr::round_any(pond_depth_l_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "pond_depth", "coiids"), drop = FALSE]

# Create acreage histogram

pond_depth_l2 <- ggplot2::ggplot(expand_pond_depth, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
if (all(is.na(EDIT_updates_filtered$pond_depth_l))) {print("No data populated in NASIS for this field.")
   ...
   pond_depth_sum_df_l <- pond_depth_na_l |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(pond_depth), pond_depth)
 
  pond_depth_sum_df_l |> dplyr::select(full_range, everything(), -range) 
}
...
pond_depth_r_acres_prepped <- data.frame(pond_depth = my_seq, row.names = NULL) |>  dplyr::rename(range = pond_depth) |> 
                        dplyr::left_join(pond_depth_acres)

pond_depth_na_r <- rbind(pond_depth_r_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), pond_depth_acres_na)

pond_depth_r_acres_prepped$acres <- ifelse(is.na(pond_depth_r_acres_prepped$acres), 0, pond_depth_r_acres_prepped$acres)
pond_depth_r_acres_prepped$range <- pond_depth_r_acres_prepped$range + 0.1

expand_pond_depth <- pond_depth_r_acres_prepped[rep(1:nrow(pond_depth_r_acres_prepped), times = plyr::round_any(pond_depth_r_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "pond_depth", "coiids"), drop = FALSE]

# Create acreage histogram

pond_depth_r2 <- ggplot2::ggplot(expand_pond_depth, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
if (all(is.na(EDIT_updates_filtered$pond_depth_l & EDIT_updates_filtered$pond_depth_h))) {print("No data populated in NASIS for this field.")
  ...
  pond_depth_sum_df_r <- pond_depth_na_r |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(pond_depth), pond_depth)
 
  pond_depth_sum_df_r |> dplyr::select(full_range, everything(), -range) |> dplyr::group_by(full_range) |> dplyr::summarise(acres = sum(acres), coiids = sum(coiids))
...
for(i in names(my_seq)){
  ...
  my_list[[i]] <- water_table.df |>  dplyr::filter(water_table %in% c("water_table_h_in", "water_table_l_in")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(water_table) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

water_table_acres <- do.call(rbind, my_list_summarised) |> 
  dplyr::mutate(numb_coiids = sapply(coiids, FUN = function(x){
    x |> stringr::str_split(",") |> unlist() |> length()
  }))

water_table_acres_na <- water_table.df |> cbind(range = NA) |>  dplyr::filter(is.na(value)) |> dplyr::group_by(water_table, range) |>  
                                dplyr::summarise(acres = sum(acres), coiids = dplyr::n()) 

# Acreage unit

acreage_unit <- ifelse(min(water_table_acres$acres[water_table_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(water_table_acres$acres[water_table_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(water_table_acres$acres[water_table_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(water_table_acres$acres[water_table_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
water_table_h_acres_prepped <- data.frame(water_table = my_seq, row.names = NULL) |>  dplyr::rename(range = water_table) |> 
                        dplyr::left_join(water_table_acres |> dplyr::filter(water_table == "water_table_h_in"))

water_table_na_h <- rbind(water_table_h_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), water_table_acres_na |> dplyr::filter(water_table == "water_table_h_in"))
...
water_table_h2 <- ggplot2::ggplot(expand_water_table_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Water table high") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Water table (inches)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
if (all(is.na(EDIT_updates_filtered$water_table_h_in))) {print("No data populated in NASIS for this field.")
  ...
  water_table_sum_df <- water_table_na_h |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(water_table), water_table)
 
  water_table_sum_df |> dplyr::select(full_range, everything(), -range) 
...
water_table_l_acres_prepped <- data.frame(water_table = my_seq, row.names = NULL) |>  dplyr::rename(range = water_table) |> 
                        dplyr::left_join(water_table_acres |> dplyr::filter(water_table == "water_table_l_in"))

water_table_na_l <- rbind(water_table_l_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), water_table_acres_na |> dplyr::filter(water_table == "water_table_l_in"))
...
water_table_l2 <- ggplot2::ggplot(expand_water_table, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
if (all(is.na(EDIT_updates_filtered$water_table_l_in))) {print("No data populated in NASIS for this field.")
      ...
      water_table_sum_df <- water_table_na_l |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(water_table), water_table)
...
water_table_r1 <- ggplot2::ggplot(water_table.df |>  dplyr::filter(water_table %in% c("water_table_l_in", "water_table_h_in")), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
water_table_r_acres_prepped <- data.frame(water_table = my_seq, row.names = NULL) |>  dplyr::rename(range = water_table) |> 
                        dplyr::left_join(water_table_acres |> dplyr::filter(water_table %in% c("water_table_l_in", "water_table_h_in")))

water_table_na_r <- rbind(water_table_r_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), water_table_acres_na |> dplyr::filter(water_table %in% c("water_table_l_in", "water_table_h_in")))

water_table_r_acres_prepped$acres <- ifelse(is.na(water_table_r_acres_prepped$acres), 0, water_table_r_acres_prepped$acres)
water_table_r_acres_prepped$range <- water_table_r_acres_prepped$range + 0.1

expand_water_table <- water_table_r_acres_prepped[rep(1:nrow(water_table_r_acres_prepped), times = water_table_r_acres_prepped$acres/acreage_unit), c("range", "water_table", "coiids"), drop = FALSE]

# Create acreage histogram

water_table_r2 <- ggplot2::ggplot(expand_water_table, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
if (all(is.na(EDIT_updates_filtered$water_table_l_in)) & all(is.na(EDIT_updates_filtered$water_table_h_in))) {print("No data populated in NASIS for this field.")
    ...
    water_table_sum_df_r <- water_table_na_r |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(water_table), water_table)
 
  water_table_sum_df_r |> dplyr::select(full_range, everything(), -range) |> dplyr::group_by(full_range) |> dplyr::summarise(acres = sum(acres), coiids = sum(coiids))
...
for(i in names(my_seq)){
  ...
  my_list[[i]] <- aspect.df |>  dplyr::filter(aspect == "aspect_rep") |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(aspect) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})
...
acreage_unit <- ifelse(min(aspect_r_acres_prepped$acres[aspect_r_acres_prepped$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(aspect_r_acres_prepped$acres[aspect_r_acres_prepped$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(aspect_r_acres_prepped$acres[aspect_r_acres_prepped$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(aspect_r_acres_prepped$acres[aspect_r_acres_prepped$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
aspect_r2 <- ggplot2::ggplot(expand_aspect_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Aspect") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Aspect (degrees)") +
  ggplot2::scale_x_continuous(limits = c(0, 360),
  breaks = seq(0,360, 30))
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- map.df |>  dplyr::filter(map %in% c("map_h_in", "map_l_in")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(map) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

map_acres <- do.call(rbind, my_list_summarised)

# Acreage unit

acreage_unit <- ifelse(min(map_acres$acres[map_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(map_acres$acres[map_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(map_acres$acres[map_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(map_acres$acres[map_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
map_h_acres_prepped <- data.frame(map = my_seq, row.names = NULL) |>  dplyr::rename(range = map) |> 
                        dplyr::left_join(map_acres |> dplyr::filter(map == "map_h_in"))
...
expand_map_h <- map_h_acres_prepped[rep(1:nrow(map_h_acres_prepped), times = plyr::round_any(map_h_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "map", "coiids"), drop = FALSE]

# Create acreage histogram

map_h2 <- ggplot2::ggplot(expand_map_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Mean Annual Precip. high") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Mean annual precip. (inches)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- map.df |>  dplyr::filter(map == "map_l_in") |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
map_l_acres_prepped <- data.frame(map = my_seq, row.names = NULL) |>  dplyr::rename(range = map) |> 
                        dplyr::left_join(map_acres |> dplyr::filter(map == "map_l_in"))
...
expand_map_l <- map_l_acres_prepped[rep(1:nrow(map_l_acres_prepped), times = plyr::round_any(map_l_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), c("range", "map", "coiids"), drop = FALSE]

# Create acreage histogram

map_l2 <- ggplot2::ggplot(expand_map_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
map_r1 <- ggplot2::ggplot(map.df |>  dplyr::filter(map %in% c("map_h_in", "map_l_in") ), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  ggplot2::ggtitle("Mean Annual Precip. representative") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("Mean Annual Precip. (inches)") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
map_r2 <- ggplot2::ggplot(expand_map_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
for(i in names(my_seq)){
  ...
  my_list[[i]] <- ffd.df |>  dplyr::filter(ffd %in% c("ffd_h", "ffd_l")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(ffd) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

ffd_acres <- do.call(rbind, my_list_summarised)
  
# Acreage unit

acreage_unit <- ifelse(min(ffd_acres$acres[ffd_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(ffd_acres$acres[ffd_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(ffd_acres$acres[ffd_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(ffd_acres$acres[ffd_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
ffd_h_acres_prepped <- data.frame(ffd = my_seq, row.names = NULL) |>  dplyr::rename(range = ffd) |> 
                        dplyr::left_join(ffd_acres |> dplyr::filter(ffd == "ffd_h"))
...
ffd_h2 <- ggplot2::ggplot(expand_ffd_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Frost Free Days high") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Frost Free Days") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
ffd_l_acres_prepped <- data.frame(ffd = my_seq, row.names = NULL) |>  dplyr::rename(range = ffd) |> 
                        dplyr::left_join(ffd_acres |> dplyr::filter(ffd == "ffd_l"))
...
ffd_l2 <- ggplot2::ggplot(expand_ffd_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
ffd_r2 <- ggplot2::ggplot(expand_ffd_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 

  ggplot2::ggtitle("Frost Free Days representative") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Frost Free Days") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- restriction.df |>  dplyr::filter(restriction %in% c("restrict_depth_h_in", "restrict_depth_l_in")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(restriction) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

restriction_acres <- do.call(rbind, my_list_summarised) |> 
  dplyr::mutate(numb_coiids = sapply(coiids, FUN = function(x){
    x |> stringr::str_split(",") |> unlist() |> length()
  }))


restriction_acres_na <- restriction.df |> cbind(range = NA) |>  dplyr::filter(is.na(value)) |> dplyr::group_by(restriction, range) |>  
                                dplyr::summarise(acres = sum(acres), coiids = dplyr::n())


# restriction_acres_na <- restriction.df |>  dplyr::filter(is.na(value) &
#                           restriction == "restrict_depth_h_in") |>  dplyr::summarise(acres = sum(acres),  coiids = n()) |>  cbind(range = NA,      
#                           restriction = NA)


# Acreage unit

acreage_unit <- ifelse(min(restriction_acres$acres[restriction_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(restriction_acres$acres[restriction_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(restriction_acres$acres[restriction_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(restriction_acres$acres[restriction_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
restriction_h_acres_prepped <- data.frame(restriction = my_seq, row.names = NULL) |>  dplyr::rename(range = restriction) |> 
                        dplyr::left_join(restriction_acres |> dplyr::filter(restriction == "restrict_depth_h_in"))

restriction_na_h <- rbind(restriction_h_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), 
                          restriction_acres_na |> dplyr::filter(restriction == "restrict_depth_h_in"))
...
expand_restriction_h <-
  restriction_h_acres_prepped[rep(
    1:nrow(restriction_h_acres_prepped),
    times = plyr::round_any(
      restriction_h_acres_prepped$acres / acreage_unit,
      accuracy = 1,
      f = round
    )
  ),
  c("range", "restriction", "coiids"), drop = FALSE]

# Create acreage histogram

restriction_h2 <- ggplot2::ggplot(expand_restriction_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = 2.5,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Restriction depth high") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Restriction depth (inches)") +
  ggplot2::scale_x_continuous(limits = c(0, 90),
  breaks = seq(0, 90, 5))
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_h_in))) {print("No data populated in NASIS for this field.")
  ...
  restriction_sum_df <- restriction_na_h |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(restriction), restriction)
 
  restriction_sum_df |> dplyr::select(full_range, everything(), -range) 
...
restriction_l_acres_prepped <- data.frame(restriction = my_seq, row.names = NULL) |>  dplyr::rename(range = restriction) |> 
                        dplyr::left_join(restriction_acres |> dplyr::filter(restriction == "restrict_depth_l_in"))

restriction_na_l <- rbind(restriction_l_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), 
                          restriction_acres_na |> dplyr::filter(restriction == "restrict_depth_l_in"))
...
expand_restriction_l <- restriction_l_acres_prepped[rep(1:nrow(restriction_l_acres_prepped), times =
                              plyr::round_any(restriction_l_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), 
                                                    c("range", "restriction", "coiids"), drop = FALSE]

# Create acreage histogram

restriction_l2 <- ggplot2::ggplot(expand_restriction_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = 2.5,
  color = "black",
  fill = "grey") + 

  ggplot2::ggtitle("Restriction low") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Restriction depth (inches)") +
  ggplot2::scale_x_continuous(limits = c(0, 90),
  breaks = seq(0, 90, 5))
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_l_in))) {print("No data populated in NASIS for this field.")
  ...
  restriction_sum_df <- restriction_na_l |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(restriction), restriction)
...
restriction_r1 <- ggplot2::ggplot(restriction.df |>  dplyr::filter(restriction %in% c("restrict_depth_h_in", "restrict_depth_l_in")), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  binwidth = bin_width,
  center = 2.5,
  color = "black",
  fill = "grey") + 

  ggplot2::ggtitle("Restriction depth representative") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("Restriction depth (inches)") +
  ggplot2::scale_x_continuous(limits = c(0, 90),
  breaks = seq(0, 90, 5))
...
restriction_r_acres_prepped <- data.frame(restriction = my_seq, row.names = NULL) |>  dplyr::rename(range = restriction) |> 
                        dplyr::left_join(restriction_acres)

restriction_na_r <- rbind(restriction_r_acres_prepped |>  dplyr::select(-coiids) |>  dplyr::rename(coiids = numb_coiids), restriction_acres_na)
...
restriction_r2 <- ggplot2::ggplot(expand_restriction_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = 2.5,
  color = "black",
  fill = "grey") + 
...
if (all(is.na(EDIT_updates_filtered$restrict_depth_h_in)) &
  ...
  restriction_sum_df_r <- restriction_na_r |>  dplyr::filter(!is.na(acres)) |> dplyr::mutate(full_range = ifelse(is.na(range), NA,
                                            paste(range, "-", range + bin_width))) |>  dplyr::arrange(!is.na(restriction), restriction)
 
  restriction_sum_df_r |> dplyr::select(full_range, everything(), -range) |> dplyr::group_by(full_range) |> 
    dplyr::summarise(acres = sum(acres), coiids = sum(coiids))
...
#### **Surface frag (2 mm. - 3 inches)**

Surface frag (2 mm. - 3 inches) prep code
```{r, message=FALSE}
...
surf_frag.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, frags_hi_2mm_3in_on_surf, frags_lo_2mm_3in_on_surf,
                                   comp_acres) |> 
  tidyr::pivot_longer(
    names_to = "surf_frag",
    values_to = "value",
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- surf_frag.df |>  dplyr::filter(surf_frag %in% c("frags_hi_2mm_3in_on_surf", "frags_lo_2mm_3in_on_surf")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(surf_frag) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

surf_frag_acres <- do.call(rbind, my_list_summarised)

# Acreage unit

acreage_unit <- ifelse(min(surf_frag_acres$acres[surf_frag_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(surf_frag_acres$acres[surf_frag_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(surf_frag_acres$acres[surf_frag_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(surf_frag_acres$acres[surf_frag_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
surf_frag1 <- ggplot2::ggplot(surf_frag.df |>  dplyr::filter(surf_frag == "frags_hi_2mm_3in_on_surf"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  ggplot2::ggtitle("Surface frag high (2 mm. - 3 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
...
surf_frag_r_acres_prepped <- data.frame(surf_frag = my_seq, row.names = NULL) |>  dplyr::rename(range = surf_frag) |> 
                        dplyr::left_join(surf_frag_acres |> dplyr::filter(surf_frag == "frags_hi_2mm_3in_on_surf"))
...
expand_surf_frag <- surf_frag_r_acres_prepped[rep(1:nrow(surf_frag_r_acres_prepped), times = 
                                    plyr::round_any(surf_frag_r_acres_prepped$acres/acreage_unit, accuracy = 1, 
                                                    f = round)), c("range", "surf_frag", "coiids"), drop = FALSE]

# Create acreage histogram

surf_frag2 <- ggplot2::ggplot(expand_surf_frag, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Surface frag high (2 mm. - 3 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Surface frag (%)") +
  ggplot2::scale_x_continuous(limits = c(0, 100),
  breaks = seq(0,100, 10))
...
**Surface frag high (2 mm. - 3 inches) acreage weighted 95th percentile** - use as 'high (actual)'
```{r, fig.show="hold", out.width="50%"}
if (all(is.na(EDIT_updates_filtered$frags_hi_2mm_3in_on_surf))) {print("No data populated in NASIS for this field.")
...
**Surface frag high (2 mm. - 3 inches) acreage weighted summary statistics**
```{r}
...
surf_frag1 <- ggplot2::ggplot(surf_frag.df |>  dplyr::filter(surf_frag == "frags_lo_2mm_3in_on_surf"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  ggplot2::ggtitle("Surface frag low (2 mm. - 3 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("Surface frag (%)") +
  ggplot2::scale_x_continuous(limits = c(0, 100),
  breaks = seq(0,100, 10))
...
surf_frag_l_acres_prepped <- data.frame(surf_frag = my_seq, row.names = NULL) |>  dplyr::rename(range = surf_frag) |> 
                        dplyr::left_join(surf_frag_acres |> dplyr::filter(surf_frag == "frags_lo_2mm_3in_on_surf"))

surf_frag_l_acres_prepped$acres <- ifelse(is.na(surf_frag_l_acres_prepped$acres), 0, surf_frag_l_acres_prepped$acres)
surf_frag_l_acres_prepped$range <- surf_frag_l_acres_prepped$range + 0.1

expand_surf_frag <- surf_frag_l_acres_prepped[rep(1:nrow(surf_frag_l_acres_prepped), 
                          times = plyr::round_any(surf_frag_l_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)), 
                          c("range", "surf_frag", "coiids"), drop = FALSE]
...
surf_frag1 <- ggplot2::ggplot(surf_frag.df |>  dplyr::filter(surf_frag %in% c("frags_hi_2mm_3in_on_surf", "frags_lo_2mm_3in_on_surf") ), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  ggplot2::ggtitle("Surface frag rep. (2mm. - 3 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("Surface frag (%)") +
  ggplot2::scale_x_continuous(limits = c(0, 100),
  breaks = seq(0,100, 10))
...
surf_frag_r_acres_prepped <- data.frame(surf_frag = my_seq, row.names = NULL) |>  dplyr::rename(range = surf_frag) |> 
                        dplyr::left_join(surf_frag_acres)
...
surf_frag2 <- ggplot2::ggplot(expand_surf_frag, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  ...
  ggplot2::ggtitle("Surface frag rep. (2 mm. - 3 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Surface frag (%)") +
  ggplot2::scale_x_continuous(limits = c(0, 100),
  breaks = seq(0,100, 10))
...
**Surface frag representative (2 mm. - 3 inches) acreage weighted 20th and 80th percentiles** - use as 'low (representative)' and 'high (representative)' 
```{r, fig.show="hold", out.width="50%"}
...
**Surface frag representative (2 mm. - 3 inches) acreage weighted summary statistics**
```{r}
...
surf_frag.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, frags_lo_gt_3in_on_surf,
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- surf_frag.df |>  dplyr::filter(surf_frag %in% c("frags_hi_gt_3in_on_surf", "frags_lo_gt_3in_on_surf")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
surf_frag_h_acres_prepped <- data.frame(surf_frag = my_seq, row.names = NULL) |>  dplyr::rename(range = surf_frag) |> 
                        dplyr::left_join(surf_frag_acres |> dplyr::filter(surf_frag == "frags_hi_gt_3in_on_surf"))

surf_frag_h_acres_prepped$acres <- ifelse(is.na(surf_frag_h_acres_prepped$acres), 0, surf_frag_h_acres_prepped$acres)
surf_frag_h_acres_prepped$range <- surf_frag_h_acres_prepped$range + 0.1

expand_surf_frag <- surf_frag_h_acres_prepped[rep(1:nrow(surf_frag_h_acres_prepped), 
                          times = plyr::round_any(surf_frag_h_acres_prepped$acres/acreage_unit, accuracy = 1,
                                                  f = round)), c("range", "surf_frag", "coiids"), drop = FALSE]
...
surf_frag_l_acres_prepped <- data.frame(surf_frag = my_seq, row.names = NULL) |>  dplyr::rename(range = surf_frag) |> 
                        dplyr::left_join(surf_frag_acres |> dplyr::filter(surf_frag == "frags_lo_gt_3in_on_surf"))

surf_frag_l_acres_prepped$acres <- ifelse(is.na(surf_frag_l_acres_prepped$acres), 0, surf_frag_l_acres_prepped$acres)
surf_frag_l_acres_prepped$range <- surf_frag_l_acres_prepped$range + 0.1

expand_surf_frag <- surf_frag_l_acres_prepped[rep(1:nrow(surf_frag_l_acres_prepped), times = surf_frag_l_acres_prepped$acres/acreage_unit), c("range", "surf_frag", "coiids"), drop = FALSE]
...
surf_frag1 <- ggplot2::ggplot(surf_frag.df |>  dplyr::filter(surf_frag %in% c("frags_hi_gt_3in_on_surf", "frags_lo_gt_3in_on_surf") ), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  ggplot2::ggtitle("Surface frag representative (3 inches +)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("Surface frag (%)") +
  ggplot2::scale_x_continuous(limits = c(0, 100),
  breaks = seq(0,100, 10))
...
expand_surf_frag <- surf_frag_r_acres_prepped[rep(1:nrow(surf_frag_r_acres_prepped), 
                            times = plyr::round_any(surf_frag_r_acres_prepped$acres/acreage_unit, 
                                                    accuracy = 1, f = round)), c("range", "surf_frag", "coiids"), drop = FALSE]
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- awc.df |>  dplyr::filter(awc %in% c("awc_h_0_40_in", "awc_l_0_40_in")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(awc) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

awc_acres <- do.call(rbind, my_list_summarised)

# Acreage unit

acreage_unit <- ifelse(min(awc_acres$acres[awc_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(awc_acres$acres[awc_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(awc_acres$acres[awc_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(awc_acres$acres[awc_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
awc_h_acres_prepped <- data.frame(awc = my_seq, row.names = NULL) |>  dplyr::rename(range = awc) |> 
                        dplyr::left_join(awc_acres |> dplyr::filter(awc == "awc_h_0_40_in"))
...
expand_awc_h <- awc_h_acres_prepped[rep(1:nrow(awc_h_acres_prepped), 
                                        times = plyr::round_any(awc_h_acres_prepped$acres/acreage_unit,
                                                                accuracy = 1, f = round)),
                                    c("range", "awc", "coiids"), drop = FALSE]

# Create acreage histogram

awc_h2 <- ggplot2::ggplot(expand_awc_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("AWC high (0 - 40 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("AWC") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
awc_l_acres_prepped <- data.frame(awc = my_seq, row.names = NULL) |>  dplyr::rename(range = awc) |> 
                        dplyr::left_join(awc_acres |> dplyr::filter(awc == "awc_l_0_40_in"))
...
expand_awc_l <- awc_l_acres_prepped[rep(1:nrow(awc_l_acres_prepped), 
                                        times = plyr::round_any(awc_l_acres_prepped$acres/acreage_unit,
                                                                accuracy = 1, f = round)), 
                                    c("range", "awc", "coiids"), drop = FALSE]

# Create acreage histogram

awc_l2 <- ggplot2::ggplot(expand_awc_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
awc_r1 <- ggplot2::ggplot(awc.df |>  dplyr::filter(awc %in% c("awc_h_0_40_in", "awc_l_0_40_in")), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  ggplot2::ggtitle("AWC representative (0 - 40 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab("# of components") + ggplot2::xlab("AWC") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
expand_awc_r <- awc_r_acres_prepped[rep(1:nrow(awc_r_acres_prepped), 
                                        times = plyr::round_any(awc_r_acres_prepped$acres/acreage_unit,
                                                                accuracy = 1, f = round)),
                                    c("range", "awc", "coiids"), drop = FALSE]

# Create acreage histogram

awc_r2 <- ggplot2::ggplot(expand_awc_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- ph.df |>  dplyr::filter(ph %in% c("pH_h_0_10in", "pH_l_0_10in")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(ph) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

ph_acres <- do.call(rbind, my_list_summarised)

# Acreage unit

acreage_unit <- ifelse(min(ph_acres$acres[ph_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(ph_acres$acres[ph_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(ph_acres$acres[ph_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(ph_acres$acres[ph_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
ph_h_acres_prepped <- data.frame(ph = my_seq, row.names = NULL) |>  dplyr::rename(range = ph) |> 
                        dplyr::left_join(ph_acres |> dplyr::filter(ph == "pH_h_0_10in"))
...
expand_ph_h <- ph_h_acres_prepped[rep(1:nrow(ph_h_acres_prepped), 
                                      times = plyr::round_any(ph_h_acres_prepped$acres/acreage_unit,
                                                              accuracy = 1, f = round)),
                                  c("range", "ph", "coiids"), drop = FALSE]

# Create acreage histogram

ph_h2 <- ggplot2::ggplot(expand_ph_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("pH high (0 - 10 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("pH") +
  ggplot2::scale_x_continuous(limits = c(min(my_seq), max(my_seq)),
  n.breaks = length(my_seq))
...
ph_l_acres_prepped <- data.frame(ph = my_seq, row.names = NULL) |>  dplyr::rename(range = ph) |> 
                        dplyr::left_join(ph_acres |> dplyr::filter(ph == "pH_l_0_10in"))
...
expand_ph_l <- ph_l_acres_prepped[rep(1:nrow(ph_l_acres_prepped), 
                                      times = plyr::round_any(ph_l_acres_prepped$acres/acreage_unit, 
                                                              accuracy = 1, f = round)), c("range", "ph", "coiids"), drop = FALSE]

# Create acreage histogram

ph_l2 <- ggplot2::ggplot(expand_ph_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
ph_r_acres_prepped <- data.frame(ph = my_seq, row.names = NULL) |>  dplyr::rename(range = ph) |> 
                        dplyr::left_join(ph_acres)

ph_r_acres_prepped$acres <- ifelse(is.na(ph_r_acres_prepped$acres), 0, ph_r_acres_prepped$acres)
ph_r_acres_prepped$range <- ph_r_acres_prepped$range + 0.1

expand_ph_r <- ph_r_acres_prepped[rep(1:nrow(ph_r_acres_prepped), 
                                      times = plyr::round_any(ph_r_acres_prepped$acres/acreage_unit, 
                                                              accuracy = 1, f = round)), c("range", "ph", "coiids"), drop = FALSE]

# Create acreage histogram

ph_r2 <- ggplot2::ggplot(expand_ph_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- ph.df |>  dplyr::filter(ph %in% c("pH_h_10_40in", "pH_l_10_40in")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
ph_h1 <- ggplot2::ggplot(ph.df |>  dplyr::filter(ph == "pH_h_10_40in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)),
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") +
...
ph_h_acres_prepped <- data.frame(ph = my_seq, row.names = NULL) |>  dplyr::rename(range = ph) |> 
                        dplyr::left_join(ph_acres |> dplyr::filter(ph == "pH_h_10_40in"))
...
ph_l_acres_prepped <- data.frame(ph = my_seq, row.names = NULL) |>  dplyr::rename(range = ph) |> 
                        dplyr::left_join(ph_acres |> dplyr::filter(ph == "pH_l_10_40in"))
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- subsurf_frags.df |>  dplyr::filter(frag %in% c("subsurf_frags_hi_2mm.3in", "subsurf_frags_lo_2mm.3in")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
...
my_list_summarised <- lapply(my_list, FUN = function(x){
  x |>  dplyr::mutate(value = as.numeric(value)) |>  dplyr::group_by(frag) |>  dplyr::summarise(range = mean(value), acres = sum(acres), coiids = toString(coiid)) |>  dplyr::ungroup()
})

subsurf_frags_acres <- do.call(rbind, my_list_summarised)

# Acreage unit

acreage_unit <- ifelse(min(subsurf_frags_acres$acres[subsurf_frags_acres$acres != 0], na.rm = TRUE) < 100, 1, 
               ifelse(min(subsurf_frags_acres$acres[subsurf_frags_acres$acres != 0], na.rm = TRUE) < 1000, 10, 
                      ifelse(min(subsurf_frags_acres$acres[subsurf_frags_acres$acres != 0], na.rm = TRUE) < 10000, 100, 
                             ifelse(min(subsurf_frags_acres$acres[subsurf_frags_acres$acres != 0], na.rm = TRUE) < 
                                      100000, 1000, 10000))))
...
subsurf_frags_h_acres_prepped <- data.frame(subsurf_frags = my_seq, row.names = NULL) |>  dplyr::rename(range = subsurf_frags) |> 
                        dplyr::left_join(subsurf_frags_acres |> dplyr::filter(frag == "subsurf_frags_hi_2mm.3in"))

subsurf_frags_h_acres_prepped$acres <- ifelse(is.na(subsurf_frags_h_acres_prepped$acres), 0, subsurf_frags_h_acres_prepped$acres)
subsurf_frags_h_acres_prepped$range <- subsurf_frags_h_acres_prepped$range + 0.1

expand_subsurf_frags_h <- subsurf_frags_h_acres_prepped[rep(1:nrow(subsurf_frags_h_acres_prepped), times = subsurf_frags_h_acres_prepped$acres/acreage_unit), c("range", "frag", "coiids"), drop = FALSE]

# Create acreage histogram

subsurf_frag_h2 <- ggplot2::ggplot(expand_subsurf_frags_h, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
  ...
  ggplot2::ggtitle("Subsurface frag high (2 mm. - 3 inches)") + ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5)) +
  ggplot2::ylab(paste0("acres (", names(acreage_unit), " of acres)")) + ggplot2::xlab("Subsurface frag (%)") +
  ggplot2::scale_x_continuous(limits = c(0, 100),
  breaks = seq(0,100, 10))
...
subsurf_frags_l_acres_prepped <- data.frame(subsurf_frags = my_seq, row.names = NULL) |>  dplyr::rename(range = subsurf_frags) |> 
                        dplyr::left_join(subsurf_frags_acres |> dplyr::filter(frag == "subsurf_frags_lo_2mm.3in"))

subsurf_frags_l_acres_prepped$acres <- ifelse(is.na(subsurf_frags_l_acres_prepped$acres), 0, subsurf_frags_l_acres_prepped$acres)
subsurf_frags_l_acres_prepped$range <- subsurf_frags_l_acres_prepped$range + 0.1

expand_subsurf_frags_l <- subsurf_frags_l_acres_prepped[rep(1:nrow(subsurf_frags_l_acres_prepped), 
                                times = plyr::round_any(subsurf_frags_l_acres_prepped$acres/acreage_unit,
                                        accuracy = 1, f = round)), c("range", "frag", "coiids"), drop = FALSE]

# Create acreage histogram

subsurf_frags_l2 <- ggplot2::ggplot(expand_subsurf_frags_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
subsurf_frags_r_acres_prepped <- data.frame(subsurf_frags = my_seq, row.names = NULL) |>  dplyr::rename(range = subsurf_frags) |> 
                        dplyr::left_join(subsurf_frags_acres)
...
expand_subsurf_frags_r <- subsurf_frags_r_acres_prepped[rep(1:nrow(subsurf_frags_r_acres_prepped), 
                            times = plyr::round_any(subsurf_frags_r_acres_prepped$acres/acreage_unit, 
                                                    accuracy = 1, f = round)),
                            c("range", "frag", "coiids"), drop = FALSE]

# Create acreage histogram

subsurf_frag_r2 <- ggplot2::ggplot(expand_subsurf_frags_r, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
for(i in names(my_seq)){
  ...
  if(i == 0){
      ...
      my_list[[i]] <- subsurf_frags.df |>  dplyr::filter(frag %in% c("subsurf_frags_hi_gt_3in", "subsurf_frags_lo_gt_3in")) |>  dplyr::filter(value >= as.numeric(i) & value <= as.numeric(i) + bin_width)
  
} else {
  
    my_list[[i]] <- subsurf_frags.df |>  dplyr::filter(frag == "subsurf_frags_hi_gt_3in") |>  dplyr::filter(value > as.numeric(i) & value <= as.numeric(i) + bin_width)
...
subsurf_frags_h_acres_prepped <- data.frame(subsurf_frags = my_seq, row.names = NULL) |>  dplyr::rename(range = subsurf_frags) |> 
                        dplyr::left_join(subsurf_frags_acres |> dplyr::filter(frag == "subsurf_frags_hi_gt_3in"))

subsurf_frags_h_acres_prepped$acres <- ifelse(is.na(subsurf_frags_h_acres_prepped$acres), 0, subsurf_frags_h_acres_prepped$acres)
subsurf_frags_h_acres_prepped$range <- subsurf_frags_h_acres_prepped$range + 0.1

expand_subsurf_frags_h <- subsurf_frags_h_acres_prepped[rep(1:nrow(subsurf_frags_h_acres_prepped), 
        times = plyr::round_any(subsurf_frags_h_acres_prepped$acres/acreage_unit, 
                                accuracy = 1, f = round)),
        c("range", "frag", "coiids"), drop = FALSE]
...
subsurf_frags_l_acres_prepped <- data.frame(subsurf_frags = my_seq, row.names = NULL) |>  dplyr::rename(range = subsurf_frags) |> 
                        dplyr::left_join(subsurf_frags_acres |> dplyr::filter(frag == "subsurf_frags_lo_gt_3in"))

subsurf_frags_l_acres_prepped$acres <- ifelse(is.na(subsurf_frags_l_acres_prepped$acres), 0, subsurf_frags_l_acres_prepped$acres)
subsurf_frags_l_acres_prepped$range <- subsurf_frags_l_acres_prepped$range + 0.1

expand_subsurf_frags_l <- subsurf_frags_l_acres_prepped[rep(1:nrow(subsurf_frags_l_acres_prepped), 
    times = plyr::round_any(subsurf_frags_l_acres_prepped$acres/acreage_unit, accuracy = 1, f = round)),
    c("range", "frag", "coiids"), drop = FALSE]

# Create acreage histogram

subsurf_frag_l2 <- ggplot2::ggplot(expand_subsurf_frags_l, ggplot2::aes(x = range)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", stringr::str_wrap(coiids, 40))),  
  binwidth = bin_width,
  center = min(my_seq) + bin_width/2,
  color = "black",
  fill = "grey") + 
...
expand_subsurf_frags_r <- subsurf_frags_r_acres_prepped[rep(1:nrow(subsurf_frags_r_acres_prepped), times = subsurf_frags_r_acres_prepped$acres/acreage_unit), c("range", "frag", "coiids"), drop = FALSE]
