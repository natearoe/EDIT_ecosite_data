...
restriction_h1 <- ggplot2::ggplot(restriction.df |>  dplyr::filter(restriction == "restrict_depth_h_in"), ggplot2::aes(x = value)) + ggplot2::geom_histogram(ggplot2::aes(text = paste("coiid(s):", coiid)), 
  ...
  breaks = seq(0, 90, 5)[seq(1, length(my_seq), by = 2)])
...
if (all(is.na(EDIT_updates_filtered$restrict_kind))) {print("No data populated in NASIS for this field.")
  ...
  EDIT_updates_filtered |>  dplyr::select(coiid, restrict_kind, restrict.kind_acres) |>  dplyr::group_by(restrict_kind) |>  
    dplyr::summarise(acres = sum(restrict.kind_acres),  '# components' = dplyr::n()) |>  dplyr::arrange(desc(acres)) |>  as.data.frame()
...
In some situations, the depth to restriction is different than the soil depth. For example, permafrost, abrupt textural changes, and salts constitute restrictions beyond which soil would continue. In these cases, restriction depth and soil depth would differ. In contrast, soil would end at a bedrock restriction and therefore restriction depth and soil depth would be the same. 

The vast majority of NASIS component horizons have RV values but do not have high and low values. Given that this report relies on component high/low values (see Important Discussion section at the beginning of the document for details), soil depth cannot be summarized in the same way as other sections in this document. 

If you have reason to believe that soil depth is different than restriction depth, you will have to use your own judgement in deciding how to utilize the information summarized for this section. Most component/high values will likely be missing. Representative values will not be reported as the combined component high/low as is the case in all other sections of this report. Instead, actual component RV values will be reported in the soil depth section. No standardized values will be calculated for populating EDIT. 
...
my_factor <- diff(range(c(EDIT_updates_filtered$soil_depth_h_in, 
             EDIT_updates_filtered$soil_depth_l_in), na.rm = TRUE))/my_vector
...
soil.depth.df <-
  EDIT_updates_filtered |>  dplyr::select(coiid, soil_depth_h_in, soil_depth_l_in, comp_acres) |> 
  tidyr::pivot_longer(
    names_to = "soil.depth",
    values_to = "value",
    cols = c("soil_depth_h_in", "soil_depth_l_in")) |> 
  dplyr::rename(acres = comp_acres)

# Sequence for axes

my_seq <- seq(plyr::round_any(min(EDIT_updates_filtered$soil_depth_l_in, na.rm = TRUE), round_by, f = floor),
  plyr::round_any(max(EDIT_updates_filtered$soil_depth_h_in, na.rm = TRUE), round_by, f = ceiling), bin_width)

my_seq <- if(min(my_seq) == min(EDIT_updates_filtered$soil_depth_l_in, na.rm = TRUE) &
             min(EDIT_updates_filtered$soil_depth_l_in, na.rm = TRUE) - bin_width >= 0){
  seq(plyr::round_any(min(EDIT_updates_filtered$soil_depth_l_in, na.rm = TRUE), round_by, f = floor) - bin_width,
      plyr::round_any(max(EDIT_updates_filtered$soil_depth_h_in, na.rm = TRUE), round_by, f = ceiling), bin_width)
} else{
