...
Concatenate your MLRAs of interest. There can be one or more. 
```{r}
MLRAs <- c("022", "018")
...
MLRAs_vector <- NULL
 ...
 for(i in seq(MLRAs)){
  MLRAs_new <- stringr::str_subset(ecosite_df$Ecosite, pattern = paste("(?<=R|F{1})", MLRAs[i], sep = ""))
...
```{r}
paste0("c(", paste(MLRAs_vector |> paste(collapse = ", ")), ")")
```
